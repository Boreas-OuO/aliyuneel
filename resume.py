...
def main():
    model_path = './save_models/model_0.pth'
    batch_size = 32
    dataloader_path = f'./save_models/batchify/dftloader.pth'
    lr=1e-4
    current_dir = os.path.dirname(os.path.realpath(__file__))
    os.chdir(current_dir)
    if os.path.exists(dataloader_path):
        dft_dataset = torch.load(dataloader_path)
    else:
        etdata = pd.read_csv('./ETTm2.csv')
        ot = etdata['OT'].values
        dft_dataset = DFTDataset(ot,norm=False)
        torch.save(dft_dataset,dataloader_path,pickle_protocol=5)
    ...
    for epoch in range(start_epoch,num_epochs):
        ...
        for inputs, targets in tqdm(dataloader, 
            ...
            if count< train_size:
                ...
                norm_train_loss += loss.item() * inputs.size(0)
                running_loss += batch_loss.item() * inputs.size(0)
            elif count>= train_size and count< train_size+val_size:
                # model.eval()
                outputs = model(inputs)
                norm_vali_loss += criterion(outputs,targets).item() * inputs.size(0)
                invtrans_outputs = dft_dataset.inverse_minmaxnorm(outputs)
                invtrans_targets = dft_dataset.inverse_minmaxnorm(targets)
                ...
                model.eval()
                outputs = model(inputs)
                norm_test_loss += criterion(outputs,targets).item() * inputs.size(0)
                invtrans_outputs = dft_dataset.inverse_minmaxnorm(outputs)
                invtrans_targets = dft_dataset.inverse_minmaxnorm(targets)
                loss = criterion(invtrans_outputs, invtrans_targets)
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, Train_Loss: {epoch_loss:.4f}/{norm_train_loss:.4f}, \
              Vali_Loss:{vali_loss:.4f}/{norm_vali_loss:.4f}, Test_Loss:{test_loss:.4f}/{norm_test_loss:.4f}")
        if epoch %10 ==0:
