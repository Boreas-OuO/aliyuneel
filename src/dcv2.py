...
class Best:
    def __init__(self, path, name,pred_len):
        self.name = name
        self.pred_len = pred_len
        self.load_template()
        index = 0
        self.best_path = path + f'exp_{index}/'
        while os.path.exists(self.best_path):
            index+=1 
            self.best_path = path + f'exp_{index}/'
        os.mkdir(self.best_path)
        self.test_log = self.Template
        self.vali_log = self.Template
        self.test_path = self.best_path + 'test.json'
        self.vali_path = self.best_path + 'vali.json'
        self.test_best = 100
        self.vali_best = 100
    ...
    def save_vali_log(self, vali_loss, mse, mae, epoch):
        if vali_loss < self.vali_best:
            self.vali_best = vali_loss
            self.vali_log[f"{self.name}"][f"{self.pred_len}"]["MSE"] = mse
            self.vali_log[f"{self.name}"][f"{self.pred_len}"]["MAE"] = mae
            with open(self.vali_path, 'w') as f:
                json.dump(self.vali_log, f, indent=4)
...
class DFTTRI:
    ...
    def load_data(self, ot, name='edt', batch_size=256):
        ...
        self.best = Best(self.log_path,name,pred_len=self.pred_len)
    ...
    def train(self):
        ...
        with tqdm(total=self.num_epochs, desc='Training', ncols=100, leave=False) as pbar:
            for self.epoch in range(self.num_epochs):
                ...
                self.best.save_vali_log(v_l,v_mse,v_mae,self.epoch)
                t_l,t_mse,t_mae = self.test()
                self.best.save_test_log(t_l,t_mse,t_mae,self.epoch)
                pbar.set_description(f'Training (Loss: {loss:.4f})')
                pbar.update(1)
                if self.epoch % 50 == 0 and self.epoch:
                    # 保存模型
                    save_path = f"./save_models/{self.name}"
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    torch.save(self.model.state_dict(), f"./save_models/{self.name}/model_{self.epoch}.pth")
